...
def process_row(row, token, session):
    ...
    return {'instruction': translated_instruction, 'response': translated_response}
...
def main():
    ...
    df = df[df.category == 'general_qa']  # open-qa
    file_name = 'general_question'
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            ...
            for future in tqdm(as_completed(future_to_row), total=len(future_to_row)):
                ...
                if result:
                    ...
                    if save_row % 100 == 0:
                        print(content['instruction'][-5:], content['response'][-5:])
